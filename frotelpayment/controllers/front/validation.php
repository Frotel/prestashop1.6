...
class FrotelPaymentValidationModuleFrontController extends ModuleFrontController
{
	public function postProcess()
	{
		...
		$Cur = Configuration::get('CurrencyDefault');
		...
		elseif($carrier == 3){$buytype = 2; $sendtype = 1; }
		elseif($carrier == 4){$buytype = 2; $sendtype = 2; }
		...
		foreach ($products as $key ) {
			...
			if($Cur == 2) $price = $key["price"] * 10;
			else $price = $key["price"];
			$basket[] = array(
...
//		die();
		...
		{	
			...
			$this->module->validateOrder($cart->id, Configuration::get('PS_OS_FROTEL_PAYMENT'), $total,
			...
			Tools::redirect($this->context->link->getModuleLink('frotelpayment', 'payment'));
		}else
		{
			$this->module->validateOrder($cart->id, Configuration::get('PS_ERR_FROTEL_PAYMENT'), $total,
				$this->module->displayName, NULL, $extra_vars, (int)$currency->id, false, $customer->secure_key);
		...
		if(isset($ar["result"]["factor"]["banks"]))
		{
			...
			Tools::redirect($this->context->link->getModuleLink('frotelpayment', 'banks'));
		}else{
