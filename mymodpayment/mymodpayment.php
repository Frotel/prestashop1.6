...
class MyModPayment extends PaymentModule
{
	...
	public function uninstall()
	{
		...
		Configuration::deleteByName('PS_OS_MYMOD_PAYMENT');
		Configuration::deleteByName('PS_PAY_MYMOD_PAYMENT');
		Configuration::deleteByName('PS_ERR_MYMOD_PAYMENT');
	...
	public function installOrderState()
	{
		...
		if (Configuration::get('PS_PAY_MYMOD_PAYMENT') < 1)
		{
			$order_state = new OrderState();
			$order_state->send_email = true;
			$order_state->module_name = $this->name;
			...
			{
				// We save the order State ID in Configuration database
				Configuration::updateValue('PS_PAY_MYMOD_PAYMENT', $order_state->id);
		...
		if (Configuration::get('PS_ERR_MYMOD_PAYMENT') < 1)
		{
			$order_state = new OrderState();
			$order_state->send_email = true;
			$order_state->module_name = $this->name;
			...
			{
				// We save the order State ID in Configuration database
				Configuration::updateValue('PS_ERR_MYMOD_PAYMENT', $order_state->id);
