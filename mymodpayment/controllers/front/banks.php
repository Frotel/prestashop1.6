...
class MyModPaymentBanksModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
		...
		if(isset($_GET["status"]) && $_GET["status"] == "orderpay")
		{
			...
			if($listbanks["code"] == 0)
			{
				$this->context->smarty->assign(array(
					...
					'factor' => $newCookie->__get("factor"),
					//'bankId' => $_GET["id"],
					'callback' => $this->context->link->getModuleLink('mymodpayment', 'returnbank'),
					'ajaxurl' => _MODULE_DIR_ . $this->name . 'mymodpayment/views/ajax/ajaxpaymentfrotel.php?secure=' . Tools::encrypt($this->name),
					'webservice' => Configuration::get('WSF'),
					'imgdir' => _MODULE_DIR_.$this->name.'mymodpayment/views/img/loading.gif',
				));
			}
...
//href="{$link->getModuleLink('mymodpayment', 'bank')}?id={$topic.id}&status={$continuepay}"
		// Set template
